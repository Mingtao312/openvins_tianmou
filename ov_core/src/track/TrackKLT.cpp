...
void TrackKLT::feed_new_camera(const CameraData &message) {
  ...
  for (size_t msg_id = 0; msg_id < num_images; msg_id++) {
    ...
    // }
    cv::Mat img = message.images.at(msg_id);  // 直接使用原图
    if (img.type() == CV_32FC1) {
            // cv::normalize(image, image, 0, 255, cv::NORM_MINMAX); // 归一化到0-255
            // cv::resize(img, img, cv::Size(h*2, w), cv::INTER_LINEAR);
            ...
            // 使用合并的掩码保留符合条件的像素
            cv::Mat mask1, mask2;
            cv::inRange(img, lowThreshold1, highThreshold1, mask1);
            cv::inRange(img, lowThreshold2, highThreshold2, mask2);
            ...
            // 使用合并的掩码保留符合条件的像素
            cv::Mat filtered;
            img.copyTo(filtered, combinedMask);

            // 2. 做线性变化
            filtered = (filtered + 50.0f) * 3.0f;

            // 3. 转换成 8-bit
            cv::Mat filtered_u8;
            filtered.convertTo(filtered_u8, CV_8UC1);

            // 4. 输出类型确认
            std::cout << "Filtered_u8 type: " << filtered_u8.type() << std::endl;
            img = filtered_u8;
        }
